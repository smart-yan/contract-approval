...
4. **结构信息由 ``Paragraph.block_type`` 显式给出**，不从文本反推。
   表格里也常出现「1. 付款阶段」这类看似条款编号的文本，如果下游只看文本，
   就会把它误判成条款起点 —— 所以"这一条到底是段落还是表格行"必须是数据，不是猜测。
...
class Paragraph(BaseModel):
    ...
    index: int = Field(
        ...
        "⚠️ ``block_type`` 不参与编号，序号语义不受它影响"
    )
    block_type: Literal["PARAGRAPH", "TABLE_ROW"] = Field(
        description="这一条内容来自哪里：``PARAGRAPH`` = 正文段落；"
        "``TABLE_ROW`` = 表格的一行（单元格文本用制表符连接）。"
        "**这是结构信息，不是从文本猜出来的** —— 解析器遍历文档时本来就知道，"
        "因此不能再用“文本里有没有制表符”去反推（单列表格就没有制表符）。"
        "下游：落库时对应 ``document_block.block_type``；P7 条款识别据此跳过表格行"
    )
    text: str = Field(
